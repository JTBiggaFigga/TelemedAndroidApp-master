...
public class Bp_ForaD30FCommunicator extends BtSppCommunicator {

    private static volatile Bp_ForaD30FCommunicator foraBpD30FCommunicator;

    private static void log(final String str) {
        ALog.log(Bp_ForaD30FCommunicator.class, str);
    }

    public static BtSppCommunicator getInstance(final int vitalType) {

        if(foraBpD30FCommunicator == null) {
            foraBpD30FCommunicator = new Bp_ForaD30FCommunicator();
        }

        foraBpD30FCommunicator.setVitalType(vitalType);

        return foraBpD30FCommunicator;
    }
    ...
    public void communicate(final BluetoothSocket sck, final InputStream in, final OutputStream out, BluetoothDevice dev, WeakReference<BTReadingsJsInterface> callerInterfaceWR) {
        ...
        try
        {
            ...
            String reading = (String) getNthReading(n);
            ...
            if(reading.contains("DEVICE_READING_MEMORY_EMPTY"))
            {
                // POWER IT OFF
                log("POWEROFF");
                powerOffSensor();

                setCurrentMiniMessage("Blood Pressure Memory is Empty.");
            }
            else
            {
    ...
    public Object getNthReading(int index) throws IOException
    {
        ...
        {
            setFinalReadingMessage("Memory Empty.\nPlease take another reading.");
        ...
        switch(type)
        {
            case TYPE_BP://BP
            {
                ...
                setFinalReadingMessage("BP: " + sysI + "/" + diaI + " mmHg");

                displayStr = "BP: " + sysI + "/" + diaI;

                return sysI+","+diaI+","+ppm + ","+displayStr;
            }
            case TYPE_SUGAR:// Sugar
            {
                ...
                setFinalReadingMessage("BP Memory Empty.");
                return "DEVICE_READING_MEMORY_EMPTY";
            }
            default:
            {
